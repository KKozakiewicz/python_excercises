...
class BJ_Game(object):
    ...
    def play(self):
        #sprawdź, czy w talii jest co najmniej tyle kart, żeby rozdać graczom i rozdającemu
        print("Liczba kart w talii: " + str(self.deck.cards_in_deck))
        if self.deck.cards_in_deck < 2*(len(self.players)+1):
	...
	# rozdaj każdemu początkowe dwie karty
        ...
        if not self.still_playing:
            ...
            if self.deck.cards_in_deck:
               self.__additional_cards(self.dealer, self.deck)
            else:
               print("Zabrakło dodatkowych kart dla rozdającego - kończymy grę")
